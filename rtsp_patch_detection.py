...
try:
    while True:
        # Instead of using just the last frame, accumulate frames over a short interval.
        flush_start = time.time()
        frame_buffer = []
        flush_duration = 2  # Duration (in seconds) over which frames are accumulated
        ...
        # Concatenate the buffered frames horizontally.
        # (For vertical concatenation, change axis=1 to axis=0)
        concatenated_frame = np.concatenate(frame_buffer, axis=1)

        # --- Apply contrast enhancement using CLAHE ---
        lab = cv2.cvtColor(concatenated_frame, cv2.COLOR_BGR2LAB)
        l, a, b = cv2.split(lab)
        ...
        # Run sliding window detection on the concatenated image
        cosine_sim, knife_detected = sliding_window_detection(image, patch_size=224, stride=112, threshold=0.270)
        ...
        # Display the enhanced frame (composite image)
        cv2.imshow('RTSP Stream', enhanced_frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        
        # Short wait before processing the next batch of frames
        time.sleep(2)
